...
#import "ITPainter.h"
#import "ITGesturesViewTransformer.h"


@interface ITMediaEditor ()

@property (weak, nonatomic) ITEditingAreaView *areaView;
//@property (weak, nonatomic) CALayer *objectLayer;
@property (weak, nonatomic) UIView *objectView;

@property (strong, nonatomic) ITPainter *painter;
@property (strong, nonatomic) ITGesturesViewTransformer *viewTransformer;
...
@implementation ITMediaEditor


- (instancetype)initWithArea:(ITEditingAreaView *)view {
    if (self = [super init]) {
        self.areaView = view;
        self.painter = [[ITPainter alloc] initWithCanvas:view];
        self.viewTransformer = [ITGesturesViewTransformer new];
    }
...
- (void)cancelAction {
    ...
    if (self.state == ITMediaEditorStatePaint) {
        [self.painter cancelAll];
    }
}
...
- (void)selectCurrentObject:(CGPoint)point {
    ...
    if (self.state == ITMediaEditorStateDefault) {
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)recognizer {
    ...
    if (self.state == ITMediaEditorStateDefault && self.objectView) {
    ...
    } else if (self.state == ITMediaEditorStatePaint) {
